...
echo ${SEP}
echo "${LOG} Check if server already bootstrapped"
...
echo ${SEP}
echo "${LOG} Checking prerequisites"
...
echo ${SEP}
echo "${LOG} Add Python3 backport install to default PATH"
...
echo ${SEP}
echo "${LOG} Generate key for Github/host interactions"
...
echo ${SEP}
echo "  Register this key   :  $(cat /root/.ssh/id_github.pub)"
echo "  as a deploy key here: https://github.com/la-francaise-com/ansible-operation-center/settings/keys"
echo ${SEP}
read -p "Then press <ENTER> to continue" </dev/tty

echo ${SEP}
echo "${LOG} Clone ansible-operation-center repository"
git clone git@github.com:la-francaise-com/ansible-operation-center.git

echo ${SEP}
echo "${LOG} Install pip requirements"
pip3 install -U -r ansible-operation-center/requirements.txt

echo ${SEP}
echo "${LOG} Mark server as bootstrapped"
mkdir -p /etc/ansible/facts.d
echo "{\"planted\": true}" > /etc/ansible/facts.d/seed.fact

echo ${SEP}
echo "${LOG} Server needs a reboot... now. (HINT: type 'reboot' and press <ENTER>)"
echo ${SEP}
